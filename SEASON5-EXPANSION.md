...
**Phase 4 (next) — Producers & ISRU.** The auto-extractor buildings (finalized structures yielding into the owner's inventory on `tick % P`, the `AUTO_MINE` precedent) — CO₂ collectors, ice wells, MOXIE, Sabatier/methalox loop — so the big colony/terraform bills are fed by infrastructure, not hand-mining; plus continuous upkeep sinks (Mars dust-storm brownout, Venus acid integrity), domes/berms/aerostat stacked-incremental builds, and the in-situ ascent `methalox` refuel.
