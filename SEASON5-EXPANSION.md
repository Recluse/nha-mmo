...
**Phase 5 — Living economy. ✅ SHIPPED 2026-08-29 (commit 5b7984f).** Converter/refinery producers (consume→produce: sabatier co2+mars_ice→methalox, perchlorate scrubber→o2, Bosch co2+H₂→water — `run_producers` honours a `consume` dict, idle when the owner is short); methalox as a launch fuel (FUEL_CLIMB ×3 — R6, closing in-situ ascent); the Venus acid-shield upkeep sink (`body_upkeep`: a complete Aphrodite Terrace corrodes −1/30 ticks, gating Venus terraforming at integrity 0 until re-funded). Seed fp unchanged; path deterministic (fp `ddb00884d40cfb30`). **Still open (a Phase 5b/6):** sustained-power terraform stages, Mars dust-storm effect on colonies, a spectator Colonies/Terraform dashboard tab, moon bases in the 3D body view, Phobos/Deimos textures, teaching scripted barons to colonise.
