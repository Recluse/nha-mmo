...
**Phase 1 — Reaching the moons.** `depart`/`land_body` verbs (moons only), `advance_transits`, the Δv formula, `orbital_engine` stamped at `finalize`, the `orbital_decay` beyond-LEO guard, launch windows, `c_regolith` mining + fuel-depot loop, new craftables. Additive attrs only. Re-run the seed (unchanged); **pin a new expansion-seed fingerprint** running a full depart→transit→arrive.

**Phase 2 — Mars & Venus reaching.** `heat_shield`/`acid_skin` craftables, EDL (Mars) and aerocapture (Venus) in `land_body`, body milestones, the ISRU `methalox` loop. Extend `IntentIn._verb_shape`.

**Phase 3 — Colonies.** Moon Forward Base, Ares Base, Aphrodite Terrace co-op boards (the `(shape, body)` lookup fix), producer extractors, domes/berms/aerostat envelopes, `_body_status` read surfaces + spectator tabs, dust-storm and acid-upkeep sinks.

**Phase 4 — Terraforming.** The Mars Greenhouse Program (warm/thicken/water/biosphere) and Venus Surface Terraform (shade/freeze/water/light), their installations (mirrors, warming/greenhouse factory, processors, L1 magnetic shield, comet redirect, sunshade/statites, Bosch reactors, soletta), the integer planetary index, sustained-power stages, one-time `count==0`-guarded deposit spawns.

**Phase 5 — The Solar Accord.** Meta-win detection, the grand pool, "Solar Architect", `world.era → 'accord'` post-game flip, prestige titles.
