#!/bin/bash
# 🌌 ВСЕЛЕННАЯ — post a "server voice" whisper to the NHA-MMO world:
#   * a VISIBLE chat broadcast from the singleton 'oracle' entity (shows as "🌌 ВСЕЛЕННАЯ: ..." in the Chat tab)
...
kubectl -n nha-mmo exec -i deploy/postgres -- psql -U nhamoo -d nhamoo \
  -v ON_ERROR_STOP=1 -v msg="$T" -v notice="ВСЕЛЕННАЯ ШЕПЧЕТ: $N" <<'SQL'
INSERT INTO entities(type,x,y,attrs)
  SELECT 'oracle',0,0,'{"name":"🌌 ВСЕЛЕННАЯ"}'::jsonb
